...
const AddShows = () => {
  ...
  const handleSubmit = async()=>{
    try {
      ...
      const datetimedata = Object.entries(dateTimeSelection).map(([date,time]) => ({date, time}))
      //it will give me object containing the objects as {date: " ", time : []}

      const payload = {
        movieId: selectedMovie,
        showsInput : datetimedata,
  ...
  //since it will take time in loading the now playing movies from the api
            ...
            nowPlayingMovies.map(movie => (
              <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300 `} onClick={()=> setSelectedMovie(movie.id)}>
                ...
                <div className="relative rounded-lg overflow-hidden">
                  <img src={tmdb_image_base_url + movie.poster_path} alt="" className="w-full object-cover brightness-90"/>
                  <div className="text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0">
                    <div className="flex items-center gap-1 text-gray-400">
                      <StarIcon className="w-4 h-4 text-primary fill-primary"/>
                      <div>{movie.vote_average}/5</div>
                    </div> 
      ...
      {/* Display Selected Times */}
        ...
        <div className="mt-6">
            ...
            <ul className="space-y-3">
              {/* entries will convert date : [times] => [date,[times]] */}
